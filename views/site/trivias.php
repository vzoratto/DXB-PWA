<?php

/* @var $this yii\web\View */

use yii\helpers\Html;

$this->title = 'Trivias';
$this->params['breadcrumbs'][] = $this->title;
?>

<div class="full-section p-0" style="background-image:url('assets/img/fondo.jpg');">

    <div class="col-md-8 col-md-offset-2 col-xs-12 text-justify reglamento-container mt-ten">

        <div class="titulo-primario text-center">
            <h1>
                Trivias
            </h1>
        </div>
        <p class="subtitulo-trivia mt-25">1. ¿Qué es el plogging?</p>
        <p>
            <strong>a. Una acción deportiva en la cual se recogen residuos mientras se trota o corre.<br></strong>
            b. Una acción deportiva donde gana aquel que recoja más residuos mientras se trota.<br>
            c. Una acción deportiva donde combina trotar/correr con recolección de residuos sólo en
            espacios urbanos.
        </p>
        <p class="subtitulo-trivia mt-25">2. Los &quot;Puntos Verdes&quot; son bolsones ubicados en distintos puntos de la ciudad, para disponer
            residuos secos reciclables limpios como papeles, cartones, telas, vidrios, los cuales luego son
            trasladados al Complejo Ambiental Neuquén para que la Cooperativa que trabaja allí recupere
            los materiales e ingresen al mercado del reciclado de residuos, generando una fuente laboral
            genuina, y mejorando la gestión de los residuos en la ciudad.
        </p>
        <p>
            a. Falso.<br>
            <strong>b. Verdadero.</strong><br>
        </p>
        <p class="subtitulo-trivia mt-25">3. En la ciudad de Neuquén, existen puntos verdes distribuidos en varios lugares de la ciudad,
            como por ejemplo:
        </p>
        <p>
            a. Hospital Heller, Universidad Nacional del Comahue, plaza de las banderas.<br>
            <strong>b. Alto Comahue shopping, delegación municipal Alto Godoy, delegación municipal barrio
                El Progreso.<br></strong>
            c. Parque central-esquina Olascoaga y Sarmiento, estación terminal de ómnibus Neuquén, Portal
            de la Patagonia shopping.<br>

        </p>
        <p class="subtitulo-trivia mt-25">
            4. ¿Cuántas calorías promedio se estima que se puede quemar en media hora de plogging?
        </p>
            <p>
                a. de 50 a 80 calorías.<br>
                b. de 500 a 600 calorías.<br>
                <strong>c. 240 a 360 calorías.<br></strong>

            </p>
        <p class="subtitulo-trivia mt-25">
            5. ¿Cuál es el animal existente más grande del planeta?
        </p>
        <p>
            <strong>a. Ballena azul.<br></strong>
            b. Ballena franca austral.<br>
            c. Elefante africano.<br>
        </p>
        <p class="subtitulo-trivia mt-25">
            6. ¿En promedio, cuánto tarda en degradarse una botella de plástico?
        </p>
            <p>
                a. Entre 5 a 15 años.<br>
                <strong>b. Entre 100 a 1000 años.<br></strong>

                c. No se degradan.<br>
            </p>

        <p class="subtitulo-trivia mt-25">
            7. ¿Cuánto tarda en degradarse una lata de aluminio?
        </p>
            <p>
                <strong>a. Entre 80 a 200 años.<br></strong>

                b. Entre 100 a 1000 años.<br>
                c. Entre 10 a 50 años.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            8. Una tonelada de vidrio reciclado generaría:
        </p>
            <p>
                a. Dos metros cúbicos de espacio libre en el relleno sanitario.<br>
                b. El ahorro energético equivalente a mantener una lámpara encendida por dos años.<br>
                <strong>c. Todas son correctas.<br></strong>
            </p>
        <p class="subtitulo-trivia mt-25">
            9. Las pilas y baterías tienen múltiples componentes en su composición, entre ellos metales
            pesados, los cuales son considerados en su disposición final como:
        </p>
            <p>
                a. Residuos reciclables.<br>
                <strong>b. Residuos peligrosos.<br></strong>

                c. Residuos especiales.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            10. ¿Qué días se recolectan domiciliariamente los residuos secos en la ciudad de Neuquén?
        </p>
            <p>
                a. Martes y jueves.<br>
                <strong>b. Lunes y jueves.<br></strong>
                c. Lunes y miércoles.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            11. ¿Cuántos residuos orgánicos se generan en promedio por día por habitante?
        </p>
            <p>
                <strong>a. Entre 45-50 %.<br>
                </strong>
                b. Entre 15-20 %.<br>
                c. Entre 60-80 %.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            12. Dentro de los residuos considerados secos podemos encontrar por ejemplo:
        </p>
            <p>
                <strong>a. Paquete de yerba, cartón, papel, envases de plástico (PET), latas de aluminio, vidrio,
                    todos limpios.<br></strong>
                b. Restos de comida, ceniza, restos de poda, saquitos de té, filtros de café.<br>
                c. Papel higiénico, pañales, algodón, toallitas femeninas y tampones usados.<br>

            </p>
        <p class="subtitulo-trivia mt-25">
            13. Los residuos sólidos urbanos se clasifican principalmente en:
        </p>
            <p>
                <strong>a. Secos y húmedos.<br></strong>
                b. Secos, húmedos, reciclables.<br>
                c. Secos, húmedos, peligrosos.<br>
            </p>

        <p class="subtitulo-trivia mt-25">
            14. El ecosistema natural en el cual se encuentra ubicada la ciudad de Neuquén se denomina:
        </p>
            <p>
                <strong>a. Monte.<br></strong>

                b. Estepa patagónica.<br>
                c. Bosque andino.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            15. Las plantas nativas de nuestras bardas se caracterizan por:
        </p>
            <p>

                a. Tener hojas de gran tamaño.<br>
                <strong>b. Poseer diversas adaptaciones para vivir con poca agua.<br></strong>

                c. Ser de gran altura (mayor a 3 metros).<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            16. El animal más veloz del mundo, que habita nuestro ecosistema regional es:
        </p>
            <p>
                a. El aguilucho.<br>
                b. El puma.<br>
                <strong>c. El halcón peregrino.</strong>

            </p>
        <p class="subtitulo-trivia mt-25">
            17. ¿Qué técnica física implica el plogging para recolectar los residuos?
        </p>
            <p>
                <strong>a. Hacer sentadillas para poder recoger los residuos, incrementando el gasto calórico.<br></strong>
                b. Curvar la espalda para levantar los residuos.<br>
                c. Hacer &quot;burpees&quot; para recolectar los residuos.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            18.- La corriente del running ha traído como consecuencia que muchas personas sin una
            preparación técnica se lanzan a correr sin adoptar la correcta postura corporal.
        </p>
            <p>
                <strong>a.- Verdadero.<br></strong>

                b.- Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            19. Detenerse de vez en cuando para recoger una botella de plástico o el envoltorio de una
            golosina ayuda a descansar de impactos con alto potencial de lesiones.
        </p>
            <p>
                <strong>a.- Verdadero.</strong><br>
                b.- Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            20. Correr por espacios breves de tiempo al borde del agotamiento y aprovechar las pausas para
            recoger los residuos, genera umbrales anaeróbicos que repercuten en la mejora cardiovascular.
        </p>
            <p>
                <strong>a.- Verdadero.<br></strong>
                b.- Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            21.¿Qué grupos musculares podrían beneficiarse al hacer plogging?
        </p>
            <p>
                <strong>a.-Caderas y de tobillos.<br></strong>
                b.- Brazos y antebrazos.<br>
                c.- Glúteos.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            22. Una de las técnicas de plogging es “apoyarse solo en una pierna estirada, rotar la pelvis
            para acercar el torso al suelo sin doblar la espalda hasta que la mano alcance el suelo y la pierna
            contraria quede estirada a 90º del suelo”. Con esta técnica se trabaja la movilidad de los
            extensores de:
        </p>
            <p>
                <strong>a.- La cadera</strong><br>
                b.- Piernas.<br>
                c.- Rodillas.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            23. Correr sosteniendo la bolsa que contiene los residuos puede endurecer los brazos y
            fortalecer el core (músculos abdominales, lumbares, de la pelvis, los glúteos y la musculatura
            profunda de la columna)
        </p>
            <p>
                <strong>
                    a.- Verdadero.
                </strong><br>
                b.- Falso <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            24.- Para practicar plogging se debe correr con la bolsa de residuos alternando los brazos, con el
            fin de no forzar la postura para sostenerla y evitar el desequilibrio muscular.
        </p>
            <p>
                <strong>a.- Verdadero<br></strong>
                b.- Falso <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            25. Los residuos de papeles, botellas plásticas (PET), cartón y paquete de yerba limpios, son
            considerados:
        </p>
            <p>
                <strong>a.- Residuos reciclables y secos.<br></strong>
                b.- Residuos no reciclables.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            26.- Si hicieras compost en tu casa, reducirías:
        </p>
            <p>
                <strong>a.- 50% de tus residuos.<br></strong>
                b.-35% de tus residuos.<br>
                c.- 20% de tus residuos.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            27.- Según el estudio de National Climate, la raza humana si continua consumiendo de esta
            forma, se extinguirá para el año:
        </p>
            <p>
                a. 2200.<br>
                b. 2100.<br>
                <strong>
                    c. 2050.
                </strong>

            </p>
        <p class="subtitulo-trivia mt-25">
            28. Según un estudio de la Organización de las Naciones Unidas por día se extinguen, entre
            especies vegetales y animales, un total de:
        </p>
            <p>
                <strong>
                    a. 150 especies.<br>
                </strong>
                b. 20 especies.<br>
                c. 50 especies.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            29. En la ciudad de Neuquén funciona un complejo ambiental, donde se disponen los residuos
            sólidos urbanos que se generan diariamente. Dentro de éste complejo funciona una planta de
            separación de residuos, la cual es gestionada por la Cooperativa &quot;Las Emprendedoras&quot;.
        </p>
            <p>
                <strong>a.- Verdadero.</strong><br>
                b.- Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            30. ¿En la ciudad de Neuquén existen reservas naturales destinadas a cuidar la barda?
        </p>
            <p>
                <strong>a.- Si.<br></strong>
                b. No.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            31.-¿Cuándo se considera una especie en peligro de extinción?
        </p>
            <p>
                a. Cuando todos los representantes de la misma corren el riesgo de desaparecer de la faz de la
                Tierra.<br>
                <strong>
                    b. Cuando el 80% de los representantes de la misma especie corren riesgo de desaparecer
                    de la faz de la tierra.
                </strong><br>

            </p>
        <p class="subtitulo-trivia mt-25">
            32.- Entre las principales causas antrópicas de extinción de especies se mencionan:
        </p>
            <p>
                a.-Destrucción y fragmentación de sus hábitats;<br>
                b.-El cambio climático;<br>
                c.- La caza y tráfico ilegal;<br>
                d.-La introducción de especies exóticas.<br>
                <strong>e.- Todas las anteriores. <br></strong>

            </p>

        <p class="subtitulo-trivia mt-25">
            33. En el año 2015 se firmó el Acuerdo de París con el fin de limitar el calentamiento global a
            un máximo de 2 °C. Si ayudamos a que esto ocurra con nuestros cambios de hábitos, se podría
            ayudar en gran medida a proteger tanto a los osos polares como a los humanos del derretimiento
            del hielo y de la extinción de muchas especies, incluidas la humana.
        </p>
            <p>
                <strong>a. Verdadera.<br></strong>
                b. Falsa.<br>
            </p>

        <p class="subtitulo-trivia mt-25">
            34. Actualmente dos mil osos pandas viven en estado natural, dispersos en varias poblaciones
            aisladas. Su fuente de alimento es el bambú, el cual se caracteriza por ser muy sensible a la
            temperatura. Se afirma que por el calentamiento global, su hábitat se destruirá en pocos años.
        </p>
            <p>
                <strong>a. Verdadera.<br></strong>
                b. Falsa. <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            35.- Cada vez existe una mayor preocupación por la presencia de plásticos no sólo en los
            océanos, sino también en la comida e incluso el aire que respiramos. Según un estudio que
            analizó el agua contenida en 259 botellas de 11 marcas distintas y en nueve países diferentes, se
            halló la siguiente cantidad de partículas de plástico:
        </p>
            <p>
                <strong>a.- Hasta 10.000 partículas de plástico por litro de agua.<br></strong>
                b.- Hasta 200 partículas de plástico por litro de agua.<br>
                c.- Ninguna partícula.<br>
            </p>

        <p class="subtitulo-trivia mt-25">
            36.- La Organización Mundial de la Salud -OMS- ha comunicado que, existen partículas
            plásticas en:
        </p>
        <p>
            <strong>a.- El 90% de las empresas que embotellan agua.<br></strong>
            b.-El 50% de las empresas que embotellan agua.<br>
            c.- El 15% de las empresas que embotellan agua.<br>
        </p>
        <p class="subtitulo-trivia mt-25">
            37. Según un estudio de la Universidad de Victoria, Kieran D. Cox , titulado &quot;Human
            Consumption of Microplastics&quot;, al evaluar el 15% de la ingesta calórica de la población
            estadounidense, se estimó que el consumo anual de microplásticos varió entre las 39.000 a
            52.000 partículas según la edad y el sexo.
        </p>
            <p>
                <strong>a.- Verdadero<br></strong>
                b.- Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            38. Los exploradores de la expedición Five Deeps, encabezados por el empresario y
            submarinista estadounidense Victor Vescovo, descendieron hasta el punto más profundo de la
            Fosa de las Marianas (aproximadamente 11 mil metros de profundidad) y hallaron una bolsa de
            plástico y restos de envoltorios de caramelos a esa profundidad.
        </p>
            <p>
                <strong>a.- Verdadero<br></strong>
                b.- Falso<br>
            </p>

        <p class="subtitulo-trivia mt-25">
            39.- ¿De dónde proviene la mayor cantidad del plástico hallada en los océanos?
        </p>
            <p>
                <strong>a. Los ríos.<br></strong>

                b. El turismo costero.<br>
                c. Camiones de residuos que vierten su contenido en el océano.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            40. En la actualidad todas las personas vivas cargan en su cuerpo una gama diversa de
            sustancias químicas tóxicas, incluidas las mujeres embarazadas y niños lactantes.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            41. En un estudio de 2005 donde se analizaron miles de productos de cuidado personal se
            descubrió que 1/3 de ellos contenía al menos un ingrediente vinculado el cáncer.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            42. El transporte marítimo global de mercancías consume más de 140 millones de toneladas/año
            de combustible, y aporta un 30 % de las emisiones de CO2 a la atmósfera.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso. <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            43. ¿Cuántos litros de agua se necesitan para fabricar un solo microchip de 2 gr de peso?
        </p>
            <p>
                a. 2 litros.<br>
                <strong>b. 32 litros.<br></strong>
                c. 15 litros.  <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            44. La población general está expuesta a un cóctel de plaguicidas a través de la comida que
            consumimos a diario.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            45. Cerca del 75% de los alimentos que consumimos y el 90% de las plantas silvestres dependen
            de la polinización, realizada principalmente por abejas.
        </p>
            <p>
                <strong>a. Verdadero.</strong><br>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            46. Una de cada dos bocanadas de aire que respiramos proviene del océano.
        </p>
            <p>
                <strong>
                    a. Verdadero.<br>
                </strong>

                b. Falso.<br>
            </p>

        <p class="subtitulo-trivia mt-25">
            47. ¿Qué porcentaje de los océanos están protegidos?
        </p>
            <p>
                a. 20%.<br>
                b. 30%.<br>
                <strong>
                    c. 3%.<br>
                </strong>

            </p>
        <p class="subtitulo-trivia mt-25">
            48. Se estima que al menos 690 especies se han topado con residuos marinos. El 92% de estos
            “encuentros” han sido con plásticos.
        </p>
            <p>
                <strong>
                    a. Verdadero.
                </strong><br>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            49. Las profundidades oceánicas son el almacén de dióxido de carbono más grande del mundo.
            Si no existiera, las concentraciones atmosféricas de CO2 actuales serían:
        </p>
            <p>
                a. Un 15% más altas.<br>
                <strong>b. Un 50% más altas.<br></strong>
                c. Un 5% más altas.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            50. Según un informe publicado en octubre de este año por la Agencia Europea del Medio
            Ambiente, la mala calidad del aire fue responsable de la muerte prematura de 428.000 personas
            en 41 países de Europa en 2014.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            51. Los ambientes áridos y semiáridos de la Provincia de Neuquén ocupan más de la mitad del
            territorio. Según un estudio realizado hace 20 años, en que porcentaje de degradación están
            afectados estos ambientes?
        </p>
            <p>
                a. 25 %.<br>
                b. 45 %.<br>
                <strong>c. 95 %. <br></strong>
            </p>
        <p class="subtitulo-trivia mt-25">
            52. Cada semana aparece un animal (principalmente de ambientes acuáticos) muerto por acción
            directa de los plásticos, ya que los mismos provocan asfixia por ahorcamiento, inmovilización,
            intoxicación por ingesta masiva, entre otros. ¿Qué acciones se podrían realizar desde nuestro
            espacio para tratar de cambiar esta problemática?
        </p>
            <p>
                a. Disponer los plásticos en los puntos limpios de la ciudad.<br>
                b. Evitar en lo posible el consumo de productos que contengan plásticos.<br>
                <strong>c. Todas son correctas.<br></strong>

            </p>
        <p class="subtitulo-trivia mt-25">
            53. ¿Cuál fue la principal causa de los incendios descontrolados que se vienen registrando en el
            Amazonas en los últimos días?
        </p>
            <p>
                a. Causas naturales (sequías, tormentas eléctricas, etc.).<br>
                <strong>b. Expansión de la frontera agropecuaria.<br></strong>
                c. Expansión de la frontera urbana.<br>
                d. Todas son correctas.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            54. En Brasil en lo que va del año 2019 se han incrementado en un 84% los incendios en
            comparación con el año 2018, y en particular éstos incendios se han incrementado en un 54% en
            la región del Amazonas.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            55. La deforestación de la Amazonia altera el paisaje y el régimen de precipitaciones en la
            región, con consecuencias incalculables. Además provoca un aumento en la emisión de gases de
            efecto invernadero causantes del calentamiento global.
        </p>
            <p>
                <strong>a. Verdadero.<br></strong>
                b. Falso.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            56. ¿Qué acciones podemos realizar desde nuestra vida cotidiana para salvar el Amazonas?
        </p>
            <p>
                a. Cambios en los hábitos de consumo alimenticio (por ejemplo: consumir menos carnes y
                productos a base de soja o aceite de palma).<br>
                b. Separar residuos domiciliarios y realizar compostaje.<br>
                c. Mantenerse informado sobre las problemáticas ambientales que ocurren en la actualidad, y
                sobre los programas y acciones de protección ambiental (a nivel global, regional y local) que se
                vienen realizando.<br>
                <strong>
                    c. Todas son correctas.<br>
                </strong>

            </p>
        <div class="titulo-primario text-center">
            <h1>
                Trivias EPAS
            </h1>
        </div>
        <p class="subtitulo-trivia mt-25">
            57. A nivel global unas 2100 millones de personas no tienen acceso a agua potable en sus
            hogares.
        </p>
            <p>
                <strong>a. Verdadero <br></strong>
                b. Falso <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            58. El 90 % de los desastres naturales están relacionados con el agua.
        </p>
            <p>
                <strong>a. Verdadero <br></strong>
                b. Falso <br>
            </p>
        <p class="subtitulo-trivia mt-25">
            59. El agua es un recurso inagotable, ya que más del 70% de la superficie de nuestro planeta
            está cubierta por agua y toda ella es apta para el consumo humano.
        </p>
            <p>
                a. Verdadero<br>
                <strong>b. Falso<br></strong>
            </p>
        <p class="subtitulo-trivia mt-25">
            60. Una canilla goteando puede derrochar hasta 200 litros de agua por día.
        </p>
            <p>
                <strong>a. Verdadero<br></strong>
                b. Falso<br>
            </p>


        <p class="subtitulo-trivia mt-25">
            61. ¿Cuántos días en promedio puede vivir un ser humano sin agua?
        </p>
            <p>
                a. Más de 10 días <br>
                <strong>b. No más de tres días<br></strong>
                c. Un día.<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            62. De total del agua que se consume diariamente en el hogar ¿Qué porcentaje se utiliza sólo en
            el baño?
        </p>
            <p>
                a. 30% <br>
                b. 50%<br>
                <strong>
                    c. 66%<br>
                </strong>

            </p>
        <p class="subtitulo-trivia mt-25">
            63. ¿Cuánta agua utiliza un lavarropas de 7 kg de capacidad en cada lavado?
        </p>
            <p>
                a. Entre 20 a 50 litros<br>
                <strong>
                    b. Entre 42 y 62 litros<br>
                </strong>

                c. Más de 200 litros<br>
            </p>
        <p class="subtitulo-trivia mt-25">
            64. ¿Cuánta agua se puede ahorrar si se cierra la canilla durante el lavado de manos y rostro?
        </p>
            <p>
                a. 3 litros por minuto<br>
                b. 5 litros por minuto<br>
                <strong>
                    c. 12 litros por minuto <br>
                </strong>

            </p>






    </div>